...
function wellKnownLayer(input: {
  ...
  return Layer.mock(AuthWellKnown.Service)({
    all: () => Effect.succeed({ [normalized]: new AuthWellKnown.Entry({ key: "TEST_TOKEN", token: "test-token" }) }),
    ...
    configs: () =>
      Effect.sync(() => {
        ...
        if (input.wellKnown && typeof input.wellKnown === "object" && "config" in input.wellKnown) {
          const config = (input.wellKnown as { config?: unknown }).config
        ...
        if (input.wellKnown && typeof input.wellKnown === "object" && "remote_config" in input.wellKnown) {
          const remoteConfig = (input.wellKnown as { remote_config?: { url?: string; headers?: Record<string, string> } })
            .remote_config
          if (remoteConfig?.url) {
            const remoteUrl = remoteConfig.url.replace("{env:TEST_TOKEN}", "test-token")
            input.seen.remote = remoteUrl
            input.seen.authorization = remoteConfig.headers?.Authorization?.replace("{env:TEST_TOKEN}", "test-token")
            docs.push({
              url: remoteUrl,
              source: remoteUrl,
